...
export default function AppShell() {
  ...
  const handleCommand = async (text) => {
    ...
    // Vente: try to parse "enregistre 5 sacs de riz vendus" or "5 sacs de riz vendus"
    const venteMatch = t.match(/(?:enregistre|enregistrer|vendre|vendu|vendus)?\s*(\d+)\s*(?:sacs?|kg|litres?)?\s*(?:de\s+)?(.+?)\s*(?:vendu|vendus)?$/);
    if (venteMatch) {
      ...
      try {
        ...
        const found = products.find((p) => p.name.toLowerCase().includes(productName));
        if (!found) {
